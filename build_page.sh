cat << 'INNER_EOF' > "src/app/(public)/page.tsx"
...
export default async function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <section className="relative px-6 py-24 md:py-32 max-w-7xl mx-auto w-full flex flex-col md:flex-row items-center gap-16">
        ...
        <div className="flex-1 relative z-10 hidden md:block">
          <div className="relative w-full max-w-sm mx-auto aspect-[1/2] rounded-[3rem] border-[8px] border-zinc-900 bg-black shadow-2xl shadow-[#00e599]/20 overflow-hidden transform rotate-6">
            <div className="absolute top-0 w-full h-8 bg-zinc-900 rounded-b-3xl" />
      ...
      <section className="px-6 py-24 max-w-7xl mx-auto w-full">
        <div className="flex flex-col lg:flex-row items-center gap-16">
          <div className="flex-1 w-full aspect-video rounded-3xl bg-zinc-900 overflow-hidden relative border border-white/5">
            <div className="absolute inset-0 bg-gradient-to-tr from-[#00e599]/20 to-transparent mix-blend-overlay" />
          ...
          {latestApps.map(app => (
            <div key={app.id} className="bg-[#0a0a0a] border border-white/10 rounded-3xl overflow-hidden flex flex-col group hover:border-[#00e599]/50 transition-colors">
              <div className="w-full aspect-[4/3] bg-zinc-900 relative p-6 flex items-end justify-center overflow-hidden">
                ...
                {app.heroImage && <img src={app.heroImage.url} alt="" className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-700" />}
                
                {/* Mockup visual inside card */}
                <div className="w-[180px] h-[350px] bg-black rounded-t-3xl border-[6px] border-zinc-800 border-b-0 relative z-20 translate-y-12">
                  <div className="w-full h-full rounded-t-2xl bg-zinc-900 p-2">
                    <div className="w-full h-full border border-white/10 rounded-xl bg-gradient-to-b from-zinc-800 to-zinc-950" />
                  </div>
                </div>
              </div>
              <div className="p-8 pt-6 flex-1 flex flex-col">
                ...
                <Link href={`/apps/${app.slug}`} className="inline-flex items-center justify-center gap-2 w-full py-3 bg-zinc-900 border border-white/10 rounded-full text-sm font-semibold hover:bg-zinc-800 transition-colors">
      ...
      <section id="faq" className="px-6 py-24 max-w-7xl mx-auto w-full border-t border-white/5 mt-12">
        <div className="flex flex-col lg:flex-row gap-16">
          ...
          <div className="w-full lg:w-2/3 flex flex-col gap-3">
            {[
              "Do you develop custom apps for clients?",
              "Where can I download your apps?",
              "Are your apps free?",
              "What technologies do you use?",
              "How long does it take to launch an app?"
            ].map((q, i) => (
              <div key={i} className="flex items-center justify-between p-6 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer">
                <span className="font-medium text-sm text-zinc-200">{q}</span>
                <Plus className="w-5 h-5 text-[#00e599]" />
              </div>
            ))}
          </div>
        </div>
      ...
      {/* 7. BOTTOM CTA */}
      <section className="px-6 py-24 max-w-7xl mx-auto w-full">
        <div className="relative w-full rounded-3xl overflow-hidden p-12 md:p-16 flex flex-col md:flex-row items-center justify-between gap-8 border border-white/10 bg-zinc-900/50">
          <div className="absolute top-0 right-0 w-full h-full bg-gradient-to-l from-[#00e599]/20 to-transparent pointer-events-none" />
